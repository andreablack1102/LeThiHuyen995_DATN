...
using LeThiHuyen995_DATN.Models.Payments;
...
namespace LeThiHuyen995_DATN.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    order.Status = 1; //1 chua thanh toan // 2/ da thanh toan 3/hoan thanh 4/huy
        ...
        public ActionResult Update(int id, int quantity)
        {
            ...
            if (cart != null)
            {
                var productItem = db.Products.FirstOrDefault(x => x.Id == id);
                var checkProduct = cart.Items.FirstOrDefault(x => x.ProductId == id);
                if (checkProduct != null)
                {
                    if (productItem.Quantity < quantity)
                    {
                        code = new { success = false, msg = "Số lượng vượt quá trong kho", code = -1 };
                    }
                    else
                    {
                        cart.UpdateQuantity(id, quantity);
                        code = new { success = true, msg = "Cập nhật thành công", code = 1 };
                    }
                }
            }
            return Json(code);
